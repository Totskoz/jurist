...
Current state: **M5 landed on master** — the full agent chain runs on real LLMs for the locked question, with M5's segment-aware routing, EU-directive escalation, graceful refusal, and a broader huur case coverage all in place. Decomposer is a single Haiku forced-tool `emit_decomposition` call that emits `huurtype_hypothese ∈ {sociale, middeldure, vrije, onbekend}`; one regen then hard-fail. Synthesizer is a Sonnet streaming `messages.stream()` call with forced tool `emit_answer`, per-request JSON-Schema `enum` on `article_id` / `bwb_id` / `ecli` (when `kind="answer"`), and post-hoc `verify_citations` (NFC + whitespace-normalized, case-sensitive strict substring, 40–500 char bounds) against article bodies + case `chunk_text`. Synthesizer emits `StructuredAnswer.kind ∈ {answer, insufficient_context}`; refusals skip citation verification. Both retrievers emit `low_confidence`; when both flag it, the synthesizer takes an early-branch refusal without the normal-path LLM call. The forced-tool constraint means Sonnet typically calls the tool immediately with **no pre-tool text** — `agent_thinking` from the synthesizer is rare (0–few events per run). On verification failure: one regen with a Dutch advisory listing the failing citations, corpus block marked `cache_control: ephemeral` so regen hits cache; still failing → `run_failed{reason:"citation_grounding"}`. Orchestrator distinguishes `anthropic.RateLimitError` from generic LLM errors, emitting `run_failed{reason:"rate_limit"}` for 429s (SDK `max_retries=8` absorbs transient bursts first). Validator remains a permanent stub.

M5 eval (2026-04-22, 5 questions against live LLMs): **3/5 pass**. Wins: AQ1 segment routing on sociale (Q2), AQ2 EU-directive escalation on oneerlijk beding (Q5), AQ8 early-branch refusal on hard out-of-scope (Q4, verzekeringsrecht). Known gaps on master — both documented in `docs/evaluations/2026-04-22-m5-suite-post.md`: (a) locked question's advice still stacks 7:248 lid 4 with 7:253 in the sociale/middeldure branch (prompt rule insufficient); (b) soft-refusal path doesn't fire for huur-adjacent out-of-scope questions (Q3 burenrecht) when retrievers report confident surface. Future work is AQ1/AQ8 hardening, not re-architecting.
...
- Probe refilter-cache workload: `uv run python scripts/dryrun_refilter.py` — counts how many cached XMLs would pass the current fence and projects embed runtime; read-only, ~30s on a warm cache. Useful before committing to a multi-hour re-ingest.
...
All 5 run on one asyncio task. Parallelization is explicitly out of scope for v1 (spec §15 decision log). The orchestrator wraps all four real-agent pumps (decomposer, statute, case, synthesizer) in try/except. Each block catches (a) the agent's specific failure exception → a dedicated `reason` (`decomposition` / `case_rerank` / `citation_grounding`), then (b) `anthropic.RateLimitError` → `reason: "rate_limit"`, then (c) generic `Exception` → `reason: "llm_error"`. The validator stub is assumed not to fail. SDK-level `max_retries=8` with exponential backoff absorbs transient 429/5xx before `RateLimitError` ever reaches the orchestrator.
...
- **Data source:** `data.rechtspraak.nl/uitspraken/zoeken` filtered on `subject=civielRecht_verbintenissenrecht` + `modified>=2024-01-01`, then a local keyword fence. Fence is 9 terms after M5: M4 baseline (`huur`/`verhuur`/`woonruimte`/`huurcommissie`) + M5 additions (`huurverhoging`/`huurprijs`/`indexering`/`oneerlijk beding`/`onredelijk beding`) to pick up EU-consumer-directive language that doesn't textually mention huur. Parent spec §8.2's original `rechtsgebied=Huurrecht` filter was wrong — no such URI exists in the taxonomy.
...
- **Call shape:** `src/jurist/agents/synthesizer.py::run` — `ctx.llm.messages.stream()` with forced tool `emit_answer`. Sonnet 4.6; `max_tokens=8192`; system prompt loaded from `llm/prompts/synthesizer.system.md` (file-based). Two `cache_control: ephemeral` breakpoints: one on the system prompt, one on the corpus content block (question + article bodies + case chunks). The instructions + regen advisory live in a second, uncached content block — the regen hits cache on the corpus and only pays for the short tail.
...
- **Events:** `agent_started` → optional `agent_thinking` × N (rare — forced tool typically skips pre-tool text; 0 on most runs, up to ~20 when Sonnet does reason out loud) → `citation_resolved` × (articles + cases) → `answer_delta` × many (synthetic word-level replay of `korte_conclusie + explanations + aanbeveling`) → `agent_finished{SynthesizerOut}`.
