...
**In scope.** Two new real agents. One new pure-helper module (`synthesizer_tools.py`). Two `schemas.py` changes (`WetArtikelCitation` adds `article_id`; `CitedCase` adds `chunk_text`). One `case_retriever.py` + `case_retriever_tools.py` change to populate `chunk_text`. Three new exception types (`DecomposerFailedError`, `CitationGroundingFailedError`, `InvalidSynthesisOutput`). One new streaming mock (`MockStreamingClient`). Three new settings (`model_decomposer`, `model_synthesizer`, `synthesizer_max_tokens`). Parent-spec amendment (§5.1/5.3/5.4/6.3/11/13/15).
...
**Added:**
- `src/jurist/agents/synthesizer_tools.py` — pure sync helpers: `build_synthesis_tool_schema`, `build_synthesis_user_message`, `verify_citations`, `_normalize`, `FailedCitation` dataclass, `InvalidSynthesisOutput` exception.
...
- `InvalidSynthesisOutput` (in `synthesizer_tools.py`) — helper-level "this single attempt was bad." Carries `failures: list[FailedCitation]` (may be empty if the failure is pre-verification, e.g., missing tool_use or schema-invalid tool_input). Caught inside the regen loop; wrapped into `CitationGroundingFailedError` on second failure.

  ```python
  class InvalidSynthesisOutput(Exception):
      def __init__(self, message: str, failures: list[FailedCitation] | None = None) -> None:
          super().__init__(message)
          self.failures = failures or []
  ```
- `InvalidDecomposerOutput` (in `decomposer.py`) — decomposer analog; wrapped into `DecomposerFailedError` on second failure.
...
tool_use = _extract_tool_use(final, "emit_answer")    # or raise InvalidSynthesisOutput
yield ("tool", tool_use.input)
```
